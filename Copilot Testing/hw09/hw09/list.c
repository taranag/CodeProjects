...
static struct user *create_user(char *name, enum user_type type, union user_data data)
{
    ...
    user->type = type;
    // Deep copy the data union
    switch (type)
    {
    case STUDENT:
        user->data = create_student(data.student.num_classes, data.student.grades, &user->data.student);
        if (user->data == NULL)
        {
            free(user->name);
            free(user);
        ...
        break;
    case INSTRUCTOR:
        user->data = create_instructor(data.instructor.salary, &user->data.instructor);
        if (user->data == NULL)
        {
            free(user->name);
            free(user);
...
static int user_equal(const struct user *user1, const struct user *user2)
{
    UNUSED_PARAMETER(user1);
    UNUSED_PARAMETER(user2);
